...
CREATE TABLE NEW_QUESTIONS(
  ...
  NEW_QUESTION_ID numeric generated always as identity(start with 1 increment by 1) not null,
  USER_ID NUMERIC,
  QUESTION_TEXT VARCHAR(255) NOT NULL,
  WATCHES NUMERIC DEFAULT 0,
  QUESTION_DATE DATE,
  
  constraint NEWQUESTION_PK primary key (NEW_QUESTION_ID),
...
CREATE TABLE NEW_OPTIONS(
  ...
  NEW_OPTION_ID numeric generated always as identity(start with 1 increment by 1) not null,
  NEW_QUESTION_ID NUMERIC,
  OPTION_TEXT VARCHAR(20),
  
  constraint NEWOPTION_PK primary key (NEW_OPTION_ID),
  FOREIGN KEY (NEW_QUESTION_ID) REFERENCES NEW_QUESTIONS(NEW_QUESTION_ID)
...
CREATE TABLE USER_WATCHING(
  ...
  CONSTRAINT USER_WATCHING_PK PRIMARY KEY (NEW_QUESTION_ID,USER_ID),
  FOREIGN KEY (USER_ID) REFERENCES USERS(USER_ID),
  FOREIGN KEY (NEW_QUESTION_ID) REFERENCES NEW_QUESTIONS(NEW_QUESTION_ID)
);
